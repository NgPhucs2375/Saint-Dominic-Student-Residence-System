...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (jwtToken) {
        ...
        router.push("/dashboard"); // Đăng nhập thành công, mở cổng vào Dashboard
      }
    } catch (err: unknown) {
      const response = err as { response?: { data?: { message?: string } | string } };
      const payload = response.response?.data;
      const message = typeof payload === "string" ? payload : payload?.message;
      setError(message || "Không thể kết nối tới máy chủ. Kiểm tra backend C# đang chạy và CORS.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div style={{ display: "flex", height: "100vh", width: "100vw", alignItems: "center", justifyContent: "center", background: C.bg }}>

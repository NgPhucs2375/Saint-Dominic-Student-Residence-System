...
// import { HouseService } from "@/infrastructure/services/houseService"; // Bật lại khi kết nối API
...
const HOUSE_COLORS = [C.blue, C.warn, C.success, C.purple, C.danger];

const HOUSE_MOCK_DATA: House[] = [
  { id: 1, name: "Nhà Giuse", numberOfMembers: 12, cap: 15, fundBalance: 3200000, state: "Hoạt động" },
  { id: 2, name: "Nhà Micae", numberOfMembers: 10, cap: 12, fundBalance: 2800000, state: "Hoạt động" },
  { id: 3, name: "Nhà Phêrô", numberOfMembers: 9, cap: 12, fundBalance: 3100000, state: "Hoạt động" },
  { id: 4, name: "Nhà Gioan", numberOfMembers: 8, cap: 12, fundBalance: 2900000, state: "Hoạt động" },
  { id: 5, name: "Nhà Phaolô", numberOfMembers: 9, cap: 12, fundBalance: 3420000, state: "Hoạt động" }
];

/* ── Các Component giao diện nhỏ của trang Nhà ── */
const Ring = ({ value, max, color, size = 56 }: RingProps) => {
  const [anim, setAnim] = useState(false);
  const r = (size - 8) / 2, circ = 2 * Math.PI * r;
  React.useEffect(() => { const t = setTimeout(() => setAnim(true), 80); return () => clearTimeout(t); }, []);
  return (
...
const DarkBtn = ({ children, onClick }: ButtonProps) => (
...
export default function HousesPage() {
  const housesReal = HOUSE_MOCK_DATA;
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(280px,1fr))", gap: 20 }}>
        {housesReal.map((h, index) => {
          const hColor = HOUSE_COLORS[index % HOUSE_COLORS.length];
